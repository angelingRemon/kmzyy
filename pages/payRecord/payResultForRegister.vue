	...
	<view class="padding-xs">
		<view class='bg-white padding-lr'>
			...
			<view class="padding flex flex-direction">
				<view class="regde_btn margin-tb-sm lg" @click="showDetail">查看挂号记录</view>
			</view>
...
<script>
	export default {
		...
		methods:{
			updateHospitalOrder(hisOrderId){
				let data = {hisOrderId:this.HisOrderId};
				this.$http.post(this.$api.uri.updatepaystatus,data).then(
				(result) => {
				}).catch(error => {
				});
			},
				...
				(result) => {
					...
					if (infoRes.code === 0) {
						...
						if(resData.PayStatus == "1"){
							this.paySucc = false;
							this.payText = "支付中";
							this.payDesc = "支付中，如您已付款，请勿重复支付，请稍后查询挂号记录!";
						}else if(resData.PayStatus == "2"){
							this.paySucc = true;
							this.payText= "支付成功";
							this.payDesc = "该挂号订单【"+this.hisOrderId+"】已支付成功，详情请查询挂号记录";
							this.updateHospitalOrder(this.hisOrderId)
						}else if(resData.PayStatus == "3"){
							this.paySucc = false;
							this.payText= "已取消";
							this.payDesc = "如您已付款，请勿重复支付，请稍后查询挂号记录";
						}

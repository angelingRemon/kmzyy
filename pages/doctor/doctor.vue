<template>
	<view class="content">
		...
		<view class="doctor_list">
			<navigator :url="'/pages/doctor/doctorDetails?DoctorCode='+item.DoctorCode" class="doctor_item" v-for="item in doctorList" :key="item.DoctorCode">
				<image src="../../static/images/z002.png" mode=""></image>
				<view class="doctor_list_right">
					<view class="doctor_list_name">
						{{item.DoctorName}} <text>{{item.DoctorLevel}} </text>
					</view>
					<view class="doctor_list_con">
						专业特长：{{item.Description}}
					</view>
				</view>
			</navigator>
		</view>

<template>
	<view class="content">
		...
		<navigator class="regde_btn"  v-if="list.length>0"
		:url="'/pages/doctor/doctorDetailsChoice?DoctorCode='
				 +DoctorCode+'&BeginDate='+BeginDate+'&EndDate='+EndDate
				 +'&SourceType=2'+'&clactiveNnm=-1'">
			预约挂号
			<image src="../../static/img/doctorDetails02.png" mode=""></image>
		</navigator>
		<navigator v-if="isShowBtn" class="regde_btn" @click="openLiaoTian(DoctorCode)">
			在线咨询
			<image src="../../static/img/doctorDetails02.png" mode=""></image>
		</navigator>
...
<script>
	...
	export default {
		...
		data() {
			...
			(e.type=='zxzx')&&(this.isShowBtn = true);
			...
			openLiaoTian(uid){
				...
				this.$http.post(this.$api.uri.imusercreate,{userId:uid}).then(res=>{
					let resultData = res.data;
					uni.hideLoading();
					if(resultData.code === 0){
						this.$store.dispatch('checkoutConversation', "c2c:"+uid+":"+this.myInfo.id);
					}else{
						uni.showToast({
							icon: 'none',
							title: '发起咨询失败'

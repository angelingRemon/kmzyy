<template>
	<view class="content">
		...
		<view class="patientadd_form">
			...
			<view class="patientadd_form_item"><view><input type="text" value="" v-model="form.CardNo" placeholder="为空则重新为您生成新的就诊卡号" /></view><text>就诊卡号</text></view>
			<!-- <view class="patientadd_form_item">
				<view class="text-right">
					 <radio-group  @change="GenderChange">
						<label>
							<radio class='blue radio' value="1"  checked="checked" />
							<text class="margin-left-sm title">男</text>
						</label>
						<label class="margin-left-sm">
							<radio class='red radio' value="2" />
							<text class="margin-left-sm title">女</text>
						</label>
					 </radio-group>
				</view>
			    <text>患者性别</text>
		...
		<view class="regde_btn" @click="submit">立即绑定</view>
	</view>
</template>
<script>
	export default {
		...
		methods: {
			...
			submit(){
				  ...
				  this.$http.request({url:this.$api.uri.createpatient,data: this.form,method:"POST",noCheckToken:true}).then(
				      (result) => {
						uni.hideLoading();
